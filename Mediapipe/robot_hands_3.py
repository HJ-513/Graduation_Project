...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks is not None:
        for res in result.multi_hand_landmarks:
            ...
            ang_ls = [thumb_cmc, thumb, index, middle, ring, pinky, angle1, angle5, angle9, angle13, angle17]
            print (ang_ls)
            
            cv2.putText(img, text=thumb_cmc, org=(int(res.landmark[1].x * img.shape[1]+ 40), int(res.landmark[1].y * img.shape[0])), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255,0,0), thickness=2)
